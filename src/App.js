import React, { Component } from 'react';
import './App.css';
import Movie from './Movie';
import Header from './Header';

const movieTitles = [
    "매트릭스",
    "올드보이",
    "secret",
    "스타워즈"
]

const moviePosters = [
    "http://ticketimage.interpark.com/Movie/still_image/V16/V1601447p_s01.gif",
    "http://artinsight.co.kr/n_news/peg/1508/thumb/a4cb4757610239a28cf0cba125b24a69_vpZESkDO68sOb6AbZkW9hFd.jpg",
    "https://upload.wikimedia.org/wikipedia/en/thumb/c/c6/Secret-Bunengshuodemimi2.jpg/220px-Secret-Bunengshuodemimi2.jpg",
    "https://upload.wikimedia.org/wikipedia/ko/thumb/6/6a/%EC%8A%A4%ED%83%80%EC%9B%8C%EC%A6%88_%EA%B9%A8%EC%96%B4%EB%82%9C_%ED%8F%AC%EC%8A%A4.jpg/250px-%EC%8A%A4%ED%83%80%EC%9B%8C%EC%A6%88_%EA%B9%A8%EC%96%B4%EB%82%9C_%ED%8F%AC%EC%8A%A4.jpg"
]

class App extends Component {
  render() {
    return (
      <div className="App">
        <Header/>
          <Movie title={movieTitles[0]} poster={moviePosters[0]}></Movie>
          <Movie title={movieTitles[1]} poster={moviePosters[1]}></Movie>
          <Movie title={movieTitles[2]} poster={moviePosters[2]}></Movie>
          <Movie title={movieTitles[3]} poster={moviePosters[3]}></Movie>
      </div>
    );
  }
}

export default App;
